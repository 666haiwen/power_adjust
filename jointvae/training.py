...
class Trainer():
    ...
    def train(self, data_loader, epochs=10, save_training_gif=None):
        ...
        for epoch in range(epochs):
            ...
            print('Epoch: {} Average loss: {:.2f}'.format(epoch + 1,
                                                          self.batch_size * self.model.dim * mean_epoch_loss))
            ...
            if self.lr_decay == True and (epoch + 1) % self.lr_update_epoch == 0:
    ...
    def _train_iteration(self, data):
        ...
        if self.num_steps % 1000 == 0:
            print('recon result: {}'.format(recon_batch[0][:20]))
        return train_loss
    ...
    def _loss_function(self, data, recon_data, latent_dist):
        ...
        # recon_data_fix = data[:, :self.model.fix_dim]
        # recon_data_mark = data[:, self.model.fix_dim:]
        # Reconstruction loss is pixel wise cross-entropy
        # recon_loss_fix = F.mse_loss(recon_data_fix.view(batch_size, -1),
        #                                 recon_data_fix.view(batch_size, -1), reduction='sum')
        # recon_loss_mark = F.binary_cross_entropy(recon_data_mark.view(batch_size ,-1),
        #                                         recon_data_mark.view(batch_size, -1))
        recon_loss = F.binary_cross_entropy(recon_data.view(batch_size, -1),
                                            data.view(batch_size, -1))
        recon_loss *= self.model.dim
        ...
        if self.model.training and self.num_steps % self.record_loss_every == 1:
            ...
            print('record_loss: [{}]  kl_loss: [{}]  loss: [{}]'\
